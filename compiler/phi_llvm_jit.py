...
"""
╔══════════════════════════════════════════════════════════════════════════════╗
║  Φ-LLVM v2 — Compilação Python→LLVM IR→ELF→VirtualAlloc                  ║
║  ══════════════════════════════════════════════════════════════════════════ ║
║                                                                             ║
║  ABORDAGEM: AST Python → LLVM IR → emit_object (ELF) → extrai código      ║
║  de seções executáveis → VirtualAlloc + ctypes call.                        ║
║                                                                             ║
║  Por que isso funciona: o MCJIT finalize_object() trava em loops no        ║
║  Windows. Mas emit_object() retorna ELF completo em <1ms. O código nas     ║
║  seções (.ltext) é position-independent e auto-contido — sem relocations.  ║
║                                                                             ║
╚══════════════════════════════════════════════════════════════════════════════╝
"""
import ast
...
# ══════════════════════════════════════════════════════════════════════════════
# §1  ELF PARSER — Extrai código executável de ELF object
# ══════════════════════════════════════════════════════════════════════════════

class ELFObjectExtractor:
    """Extrai código de máquina de um object file ELF64."""
    
    @staticmethod
    def extract_executable_sections(obj_data: bytes) -> bytes:
        """Extrai todas as seções executáveis (flags com SHF_EXECINSTR=0x4) concatenadas."""
        if len(obj_data) < 64:
            return b''
        
        # ELF64 header
        fmt = '<16sHHIQQQIHHHHHH'
        fields = struct.unpack_from(fmt, obj_data, 0)
        _, _, _, _, _, _, shoff, _, _, _, _, shentsize, shnum, shstrndx = fields
        
        # String table
        sh = shoff + shstrndx * shentsize
        st_off = struct.unpack_from('<Q', obj_data, sh + 24)[0]
        st_sz = struct.unpack_from('<Q', obj_data, sh + 32)[0]
        strtab = obj_data[st_off:st_off + st_sz]
        
        code_blobs = []
        
        for i in range(shnum):
            sec_hdr = shoff + i * shentsize
            s_type = struct.unpack_from('<I', obj_data, sec_hdr + 4)[0]
            s_flags = struct.unpack_from('<Q', obj_data, sec_hdr + 8)[0]
            s_off = struct.unpack_from('<Q', obj_data, sec_hdr + 24)[0]
            s_sz = struct.unpack_from('<Q', obj_data, sec_hdr + 32)[0]
            
            # SHF_EXECINSTR = 0x4, SHT_PROGBITS = 1
            is_exec = (s_flags & 0x4) != 0
            is_progbits = (s_type == 1)
            
            if s_sz > 0 and is_exec and is_progbits:
                code = obj_data[s_off:s_off + s_sz]
                code_blobs.append(code)
        
        return b''.join(code_blobs)
    
    @staticmethod
    def extract_function_body(obj_data: bytes) -> Tuple[bytes, int]:
        """Extrai o código principal (maior blob executável)."""
        blob = ELFObjectExtractor.extract_executable_sections(obj_data)
        if not blob:
            return b'', 0
        
        # Remove prefixo comum (setup de frame) e sufixo (padding)
        # O código principal é o maior bloco contínuo
        # Para objetos simples, é todo o blob
        return blob, len(blob)

# ══════════════════════════════════════════════════════════════════════════════
# §2  LLVM IR BUILDER (reutilizado da v1, com correções)
# ══════════════════════════════════════════════════════════════════════════════
...
class PhiLLVMIRBuilder:
    ...
    def _create_entry(self, arg_name: str = "x") -> Tuple[ir.Function, ir.IRBuilder]:
        i64 = ir.IntType(64)
        ...
        arg = self.func.args[0]
        arg.name = arg_name
        alloca = self.builder.alloca(i64, name=arg_name)
        self.builder.store(arg, alloca)
        self.locals[arg_name] = alloca
        ...
        if isinstance(node, ast.Constant):
            ...
            if isinstance(op, ast.Add):       return self.builder.add(left, right, "addtmp")
                ...
                if isinstance(node.right, ast.Constant) and isinstance(node.right.value, int):
                    ...
                    for _ in range(exp):
                        result = self.builder.mul(result, left, "powtmp")
                    return result
            ...
            if isinstance(node.op, ast.USub):
                ...
                return self.builder.icmp_unsigned("==", operand, ir.Constant(i64, 0))
    ...
    def compile_statement(self, stmt: ast.AST) -> Optional[ir.Value]:
        ...
        if isinstance(stmt, ast.Return):
            ...
            if len(stmt.targets) == 1 and isinstance(stmt.targets[0], ast.Name):
                ...
                val = self.compile_expression(stmt.value)
                if name not in self.locals:
                    self.locals[name] = self.builder.alloca(i64, name=name)
            ...
            if isinstance(stmt.target, ast.Name):
                ...
                self.builder.store(res, self.locals[name])
                return res
        
        elif isinstance(stmt, ast.Expr):
            return self.compile_expression(stmt.value)
        
        elif isinstance(stmt, ast.If):
            cond = self.compile_expression(stmt.test)
            i1 = ir.IntType(1)
            cond_bool = cond if cond.type == i1 else self.builder.trunc(cond, i1)
            ...
            then_val = self.compile_body(stmt.body)
            ...
            if else_block:
                ...
                else_val = self.compile_body(stmt.orelse)
                if not self.builder.block.is_terminated:
                    self.builder.branch(merge_block)
            ...
            self.loop_break_block = after_block
            self.compile_body(stmt.body)
            self.loop_continue_block = old_c
            self.loop_break_block = old_b
            if not self.builder.block.is_terminated:
                self.builder.branch(cond_block)
            ...
            if (isinstance(stmt.iter, ast.Call) and isinstance(stmt.iter.func, ast.Name) 
                ...
                self.locals[target_name] = iter_alloca
                ...
                self.builder.store(next_i, iter_alloca)
                self.builder.branch(cond_block)
            ...
            if self.loop_break_block:
                self.builder.branch(self.loop_break_block)
    ...
    def compile_function(self, py_source: str, arg_name: str = "x") -> Tuple[ir.Function, ir.Module]:
        try:
            ...
            try:
                tree = ast.parse(py_source, mode='eval')
            except:
                tree = ast.parse(f"def f({arg_name}):\n    return {py_source}")
        ...
        if func_def is None:
            if isinstance(tree, ast.Expression):
                ...
                func_def = wrapper.body[0]
                func_def.body = [ast.Return(value=tree.body)]
        
        self._create_entry(arg_name)
        
        if isinstance(func_def, ast.FunctionDef):
            for arg in func_def.args.args:
                if arg.arg != arg_name and arg.arg not in self.locals:
                    self.locals[arg.arg] = self.builder.alloca(ir.IntType(64), name=arg.arg)
            self.compile_body(func_def.body)
        elif isinstance(func_def, ast.Expr):
            val = self.compile_expression(func_def.value)
            self.builder.ret(val)
...
# ══════════════════════════════════════════════════════════════════════════════
# §3  PHI LLVM JIT v2 — AST→IR→ELF→VirtualAlloc
# ══════════════════════════════════════════════════════════════════════════════

class PhiLLVMJITv2:
    """JIT engine que usa emit_object (ELF) ao invés de MCJIT finalize.
    
    Vantagens:
    - Funciona com loops (sem hang no finalize_object)
    - Rápido: emit_object leva <1ms
    - Portável: ELF parsing funciona em qualquer plataforma
    """
    ...
    def compile_source(self, py_source: str, func_name: str = "f", 
                       arg_name: str = "x") -> Tuple[Callable, bytes, float]:
        """Compila Python → native callable via ELF extraction.
        
        Returns: (callable, machine_code_bytes, compile_time_ms)
        """
        ...
        module.triple = self._tm.triple
        builder = PhiLLVMIRBuilder(module, func_name)
        builder.compile_function(py_source, arg_name)
        
        # 2. IR → object file (ELF)
        llvm_mod = llvm.parse_assembly(str(module))
        llvm_mod.verify()
        obj_data = self._tm.emit_object(llvm_mod)
        
        # 3. Extrai código executável
        code, size = ELFObjectExtractor.extract_function_body(obj_data)
        
        if not code:
            raise RuntimeError(f"No executable code found in object for: {py_source[:50]}")
        
        # 4. VirtualAlloc + executa
        ...
        self._compiled[cache_key] = (native_fn, code, compile_ms)
        return native_fn, code, compile_ms
    
    def benchmark(self, py_source: str, py_fn: Callable, test_input: int = 10,
                  n_calls: int = 100_000) -> dict:
        """Benchmark: Python vs LLVM JIT."""
        native_fn, code, compile_ms = self.compile_source(py_source)
        
        # Warmup
        for _ in range(100):
            native_fn(test_input)
        ...
        native_result = native_fn(test_input)
        py_result = py_fn(test_input)
        ...
        return {
            ...
            'result_match': native_result == py_result,
            'code_bytes': len(code),
            'py_result': py_result,
...
BENCHMARKS = [
    ('square', 'def f(x):\n    return x * x', lambda x: x * x, 42, 1_000_000),
    ('cube', 'def f(x):\n    return x * x * x', lambda x: x * x * x, 42, 1_000_000),
    ('arithmetic', 'def f(x):\n    return (x + 100) * (x - 50) // 3', 
     lambda x: (x+100)*(x-50)//3, 42, 1_000_000),
    ('conditional', 'def f(x):\n    if x > 100:\n        return x // 2\n    else:\n        return x * 2',
     lambda x: x//2 if x>100 else x*2, 150, 500_000),
    ('loop_sum', 'def f(n):\n    r = 0\n    for i in range(n):\n        r += i\n    return r',
     lambda n: sum(range(n)), 1000, 10_000),
    ('loop_squares', 'def f(n):\n    r = 0\n    for i in range(n):\n        r += i * i\n    return r',
     lambda n: sum(i*i for i in range(n)), 1000, 10_000),
    ('while_sum', 'def f(n):\n    r = 0\n    while n > 0:\n        r += n\n        n = n - 1\n    return r',
     lambda n: sum(range(n,0,-1)), 1000, 10_000),
]
...
if __name__ == "__main__":
    print("╔══════════════════════════════════════════════════════════════════╗")
    print("║  Φ-LLVM v2 — Python→LLVM IR→ELF→VirtualAlloc (NO MCJIT)       ║")
    print("╚══════════════════════════════════════════════════════════════════╝")
    print()
    ...
    for name, src, py_fn, inp, n in BENCHMARKS:
        try:
            ...
            total_native += r['native_ms']
            ok = "✓" if r['result_match'] else f"✗({r['py_result']}!={r['native_result']})"
            ...
            print(f"{name:16s} | FAIL: {str(e)[:50]}")
    ...
    print()
    print("⚫ ARQUITETURA:")
    print("  Python AST → LLVM IR → emit_object(ELF) → parse seções executáveis")
    print("  → VirtualAlloc(PAGE_EXECUTE_READWRITE) → CFUNCTYPE call")
    print("  Zero dependência de linker. Funciona com loops, condicionais, while.")
